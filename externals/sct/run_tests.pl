...
#
# todo: busy wait model not implemented (also missing sct_reduce.c:merge_sp(...))
#
use strict;
use warnings;
...
#dummy subs required to execute configure_system.pl
sub set_mvar {}
sub add_mvar {}
sub define_make_variables {};

sub load_conf() {
  %conf = ();
  open(F, "< .setup_info") || die " cannot open .setup_info";
  my $line;
  while ($line = <F>) {
    if ($line =~ /\s*([^,]+):=(.*?)\s*$/) {
      $conf{$1} = $2;
      print "read conf{$1} = $conf{$1}\n" if $verbose;
    }
  }
  close(F);
}
...
sub run_prg {
  ...
  $np = 1 unless defined $np;
  $nt = 1 unless defined $nt;
  die "run_prg: undefined args" unless defined($prg) && defined($np) && defined($nt);
  die "run_prg: invalid args" unless $prg && $np && $nt;
  print "\n*** run_prg: prg=$prg, np=$np, nt=$nt ***\n";
  if (-x $conf{execute_program}) {
    my @out = `$conf{execute_program} -f $prg -p $np -t $nt`;
    print "run_prg: [out]=[@out]\n";
    return @out;
  } else {
    die "executable script expected in \$conf{execute_program}";
  }
}
...
if ($conf{mpi}) {
  if ($conf{omp}) {
    ...
    parse(run_prg("./test_hybrid.x",2,2)) && check_reports();

    $ENV{SCT_FILENAME}="abc.txt";
    $ENV{SCT_CALLSTATS}=0;
    $ENV{SCT_PROC_CHOICE}="SCT_SELECT_ALL";
    parse(run_prg("./test_hybrid.x",2,2)) && check_reports();
    ...
    $ENV{SCT_PROC_CHOICE}="SCT_SELECT_ALL";
    $ENV{SCT_THREAD_CHOICE}="SCT_SELECT_ALL";
    parse(run_prg("./test_hybrid.x",1,2)) && check_reports();

    $ENV{SCT_PROC_CHOICE}="SCT_REDUCE_ALL";
    #$ENV{SCT_PROC_CHOICE}="SCT_SELECT_ALL";
    #$ENV{SCT_THREAD_CHOICE}="SCT_SELECT_ALL";
    $ENV{SCT_THREAD_CHOICE}="0";#"SCT_REDUCE_ALL";
    #$ENV{SCT_THREAD_CHOICE}="SCT_REDUCE_ALL";
    parse(run_prg("./test_hybrid.x",1,2)) && check_reports();
    ...
    $ENV{SCT_PROC_CHOICE}="SCT_SELECT_ALL";
    $ENV{SCT_CALLSTATS}=0;
    parse(run_prg("./test_mpi.x",1)) && check_reports();
    ...
    $ENV{SCT_CALLSTATS}=0;
    parse(run_prg("./test_mpi.x",2)) && check_reports();

    $ENV{SCT_PROC_CHOICE}="SCT_REDUCE_ALL";
    parse(run_prg("./test_mpi.x",4)) && check_reports();
    parse(run_prg("./test_mpi.x",8)) && check_reports();
  }
} else {
  if ($conf{omp}) {
    # omp:
    $ENV{SCT_CALLSTATS}=0;
    parse(run_prg("./test_omp.x", 1, 4)) && check_reports();
    ...
    $ENV{SCT_CALLSTATS}=0;
    $ENV{SCT_THREAD_CHOICE}="SCT_SELECT_ALL";
    parse(run_prg("./test_omp.x", 1, 2)) && check_reports();
    ...
    # serial:
    $ENV{SCT_CALLSTATS}=0;
    parse(run_prg("./test_serial.x")) && check_reports();
