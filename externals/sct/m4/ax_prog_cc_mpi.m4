...
AC_DEFUN_ONCE([_AX_PROG_CC_MPI], [
  ...
  if test x"$_ax_prog_cc_mpi_mpi_wanted" = xyes; then
    if test -z "$CC" && test -n "$MPICC"; then
      AC_MSG_WARN([using MPICC=$MPICC as CC compiler.])
      CC="$MPICC"
    elif test -z "$CC" && test -z "$MPICC"; then
      AC_CHECK_PROGS([CC], [mpiicc mpicc mpixlc_r mpixlc hcc mpxlc_r mpxlc sxmpicc mpifcc mpgcc mpcc_r mpcc cmpicc cc gcc])
      AC_MSG_WARN([using automatically found CC=$CC as MPICC compiler.])
    elif test -n "$CC" && test -z "$MPICC"; then
...
dnl  AC_PROG_CC99
dnl use c11 compiler search instead ... should be part of future autoconf in AC_PROG_CC
  AX_PROG_CC11
  AC_C__GENERIC
