...
! use mo_algorithms,            only: svdcmp, &! singular value dec. (wrapper)
  use mo_algorithms,            only: xgesvd   ! singular value dec. (lapack)
...
!------------------------------------------------------------------------------
  function vector_times_vector (z, x) result (y)
    real(wp) ,intent(in) :: z (:)
    real(wp) ,intent(in) :: x (:)
    real(wp)             :: y
      y = dot_product(z, x)
...
!------------------------------------------------------------------------------
  subroutine scale (A, x, xr)
  real(wp) ,intent(inout)          :: A (:,:)
  real(wp) ,intent(in) ,optional   :: x (:), xr (:)
  !--------------------------------------------------
  ! scale the matrix A so that the diagonal becomes 1
  ! if x is present scale as A -> x A x^t
  ! if x,xr are present:     A -> x A xr^t
  !--------------------------------------------------
    real(wp) :: zl(size(A,1)), zr(size(A,2))
    integer  :: n, i
    n = size(A,1)

    if (.not.present(x) .and. &
        .not.present(xr)       ) then
      if (size(A,2)/= n) &
        call finish('scale','A is not square')
      do i=1,n
        if (A(i,i)/=0._wp) then
          zl(i) = 1._wp/sqrt(abs(A(i,i)))
        else
          zl(i) = 1._wp
        endif
      end do
      ...
      if (present(x)) then
        ...
        call finish('scale','x is not present, but xr')
      endif
      if (present(xr)) then
        zr = xr
      else
...
!------------------------------------------------------------------------------
  function inverse_rs (a, max_ev, min_ev, evmin, evmax) result (y)
  real(wp), intent(in)            :: a(:,:) ! input matrix
  real(wp), intent(in),  optional :: max_ev ! maximum eigenvalue to keep
  real(wp), intent(in),  optional :: min_ev ! minimum eigenvalue to keep
  real(wp) ,intent(out) ,optional :: evmin  ! minimum eigenvalue
  real(wp) ,intent(out) ,optional :: evmax  ! maximum eigenvalue
  real(wp)                        :: y (size(a,1),size(a,1)) ! inverse matrix
  !--------------------------------------------------------------------
  ! Inverts the (real symmetric) matrix a
  ! Before inversion, eigenvalues may be restricted to min_ev .. max_ev
  ! eigenvalues before inversion may be diagnosed: evmin, evmax
  !--------------------------------------------------------------------
    call check_rs (a, max_ev=max_ev, min_ev=min_ev,  &
                      evmin=evmin, evmax=evmax, inv=y)
...
!------------------------------------------------------------------------------
  !----------------------------------------------
  ! operations based on eigenvector decomposition
  ...
  real(wp) ,intent(in)  ,optional :: max_ev       ! maximum eigenvalue to keep
  real(wp) ,intent(in)  ,optional :: min_ev       ! minimum eigenvalue to keep
  real(wp) ,intent(out) ,optional :: y   (:,:)    ! modified matrix
  real(wp) ,intent(out) ,optional :: inv (:,:)    ! inverse (A)
  integer  ,intent(out) ,optional :: hits         ! number of ev. modified
  real(wp) ,intent(out) ,optional :: evmin        ! minimum eigenvalue
  real(wp) ,intent(out) ,optional :: evmax        ! maximum eigenvalue
  ...
  real(wp) ,intent(in)  ,optional :: x            ! exponent for A ** x
  !----------------------------------------------------
...
#if defined (__NEC__) || defined (__SX__) || defined (_CRAYFTN)
    integer               :: j, k
#endif
...
#if defined (__NEC__)
      ! Explicit coding for NEC nfort: inv = matmul (zw, transpose (z))
      do j = 1, n
         do i = 1, n
            inv(i,j) = 0._wp
            do k = 1, n
...
#elif defined (__SX__) || defined (_CRAYFTN)
...
!NEC$ loop_count(3)
               do k = 1, n
                  sqroot(i,j) = sqroot(i,j) + zw(i,k) * z(j,k)
               end do
            end do
         end do
      end if
...
!------------------------------------------------------------------------------
  function inverse_svd (a, max_cond_number, det) result (y)
  real(wp), intent(in)            :: a(:,:)
  real(wp), intent(in)  ,optional :: max_cond_number
  real(wp), intent(out) ,optional :: det
  real(wp)                        :: y (size(a,2),size(a,1))
  !------------------------------------------------------------------
  ! Inverts the matrix a (by singular value decomposition)
  ! The inverse is positive semidefinite. if max_cond_number is given
  ! and > 0, the maximum condition number is max_cond_number. If
  ! max_cond_number == 0., all positive singular values are used.
  ! if max_cond_number is < 0. the minimum eigenvalue is requested
  ! interactively.
  !------------------------------------------------------------------
    real(wp), dimension (size(a,1),size(a,2)) :: u
    real(wp), allocatable                     :: v (:,:)
    real(wp), dimension (size(a,2))           :: w
    integer                                   :: i, m, n, mn
    real(wp)                                  :: min_ev, ed_min_ev
!   integer                                   :: j,k !+++ NEC workaround
    ...
    !-----------------------------
!   call svdcmp(u,w,v)    ! numerical recipes
    call xgesvd(u,w,v)    ! lapack
!   call svdc  (a,u,v,w)  ! slatec
    !-------------------------------------------
    ! calculate minimum eigenvalue of old matrix
    !-------------------------------------------
    if (present(max_cond_number)) then
      min_ev = max (maxval(w(:mn))/max_cond_number, 0._wp)
    else
      min_ev = 0._wp
    endif
    !----------------------------------------------------
    ! and corresponding max. eigenvalue of inverse matrix
    !----------------------------------------------------
    if (min_ev > 0._wp) then
      ed_min_ev = 1._wp/min_ev
    else
      ed_min_ev = 0._wp
    endif
    where (w(:mn) > min_ev)
...
!write 'fingerprint' of arguments
!write(0,*)'inverse_svd:   u=',minval(u),maxval(u),sum(u)/size(u)
!write(0,*)'inverse_svd: w*v=',minval(v),maxval(v),sum(v)/size(v)

!   Original version
!   This didn't work on TSX
!   y = matmul ( v(:,:mn), transpose (u(:,:mn)))

!   This didn't work either
!do i=1,m
!do j=1,n
!y(j,i)=dot_product (v(j,1:mn),u(i,1:mn))
!end do
!end do

!   workaround:
    call dgemm ('n', 't', n, m, mn, 1._wp, v, n, u, m, 0._wp, y, n)

!write 'fingerprint' of result
!write(0,*)'inverse_svd: return',minval(y),maxval(y),sum(y)/size(y)

    if (present(det)) det = product (w(:mn))
    deallocate (v)

  end function inverse_svd
!------------------------------------------------------------------------------
  function inverse_submatrix (M, proj, max_cond_number, det) result (Y)
  !----------------------------------
  ! invert the projection of a matrix
  !----------------------------------
  real(wp), intent(in)            :: M    (:,:)
  real(wp), intent(in)            :: proj (size(M,1))
  real(wp), intent(in)  ,optional :: max_cond_number
  real(wp), intent(out) ,optional :: det
  real(wp)                        :: Y    (size(M,2) , size(M,1))
    integer, allocatable :: indices (:)
    integer              :: n, i, k
    Y = 0._wp
    n = count (proj /= 0._wp)
    if (n == 0) then
      if (present (det)) det = 1._wp
      return
    else if (n == size(proj)) then
      Y = inverse (M, max_cond_number, det)
    else
      allocate (indices (n))
      k = 1
      do i = 1, size(proj)
        if (proj(i) /= 0._wp) then
          indices (k) = i
          k = k + 1
        endif
      end do
      Y(indices,indices) = inverse (M(indices,indices), max_cond_number, det)
      deallocate (indices)
    endif
  end function inverse_submatrix
!==============================================================================
  !-------------------------------
