...
PUBLIC :: diff_minutes, sleve_split_oro, to_upper, sortrx,                    &
          uvrot2uv, uv2uvrot_vec, phi2phirot, rla2rlarot, uv2df

          ! public subroutines and functions used in the COSMO model code only
PUBLIC :: convert_month, dolph, elapsed_time, get_utc_date,                   &
          ...
          lininterp3D_xinter1D_vec, check_field_NaNs

#ifndef _DACE_  /* Disable to avoid conflicts with CALL SLEEP elsewhere */
PUBLIC :: dosleep
#endif
          ! routines named in 'use' statements, but not called
PUBLIC :: dfilt4, dfilt8, tautsp
...
SUBROUTINE sortrx_wp(nn, rdata, iindex)
...
! --------------------
  ...
  INTEGER , INTENT(IN)          ::  nn          ! length of 'data'
  REAL(KIND=wp), INTENT(IN)     ::  rdata (nn)  ! array to be sorted
  INTEGER , INTENT(INOUT)       ::  iindex (nn) ! indices for 'idata'
...
!             notation, "K").  Also modified to leave data in place and produce
!             an 'iindex' array.
!             To simplify comments, let 'rdata[ii] = rdata(iindex(ii))'.
!------------------------------------------------------------------------------
    ...
    quicksort : DO
      ...
      ! Q2: Sort the subsequence rdata[ill]...rdata[irr].
      !
      !     At this point, rdata[l] <= rdata[m] <= rdata[r] for all l < ill,
      !     r > irr, and ill <= m <= irr.
      !     (First time through, there is no rdata for l < ill or r > irr.)
      ...
      !     'ipp=(ill+irr)/2';
      !     then rearrange 'iindex(ill)', 'iindex(ipp)', and 'iindex(irr)'
      !     so the corresponding 'rdata' values are in increasing order.
      !     The pivot key, 'datap', is then 'rdata[ipp]'.
 
      ipp    = (ill+irr)/2
      indexp = iindex(ipp)
      datap  = rdata(indexp)
 
      IF (rdata(iindex(ill)) > datap) THEN
        iindex(ipp) = iindex(ill)
        iindex(ill) = indexp
        indexp      = iindex(ipp)
        datap       = rdata(indexp)
      ...
      IF (datap > rdata(iindex(irr))) THEN
        IF (rdata(iindex(ill)) > rdata(iindex(irr))) THEN
          iindex(ipp) = iindex(ill)
          iindex(ill) = iindex(irr)
        ELSE
          iindex(ipp) = iindex(irr)
      ...
      DO WHILE (ii < jj)
        ...
        ! Q3: Search for datum on left >= 'datap'
        !
        !     At this point, 'rdata[ill] <= datap'.  We can therefore start
        !     scanning up from 'ill', looking for a value >= 'datap'
        !     (this scan is guaranteed to terminate since we initially
        !      placed 'datap' near the middle of the subsequence).
        ii = ii + 1
        DO WHILE (rdata(iindex(ii)) < datap)
          ii = ii + 1
        ENDDO
 
        ! Q4: Search for datum on right <= 'datap'
        !
        !     At this point, 'rdata[irr] >= datap'.  We can therefore start
        !     scanning down from 'irr', looking for a value <= 'datap'
        !     (this scan is guaranteed to terminate since we initially
        !      placed 'datap' near the middle of the subsequence).
        jj = jj - 1
        DO WHILE (rdata(iindex(jj)) > datap)
          jj = jj - 1
        ENDDO
      ...
      !
      !     At this point, 'ii >= jj' ,
      !     'rdata[l] <= rdata[ii] == datap <= rdata[r]',
      !     for all 'ill <= l < ii' and 'jj < r <= irr'.
      !     If both subsequences are more than 'mqcut' elements long, push
      !     the longer one on the stack and go back to QuickSort the shorter;
      !     if only one is '> mqcut' elements long, go back and QuickSort it;
...
!------------------------------------------------------------------------------
  ...
  DO ii = 2 , nn
    IF (     (       rdata(iindex(ii-1)) >  rdata(iindex(ii)) )               &
        .OR. (      (rdata(iindex(ii-1)) == rdata(iindex(ii)))                &
              .AND. (      iindex(ii-1)  >        iindex(ii) ))) THEN
      indexp = iindex(ii)
      datap  = rdata(indexp)
      ipp = ii - 1
      DO
        ...
        ipp = ipp - 1
        IF (ipp > 0) THEN
          IF (     (       rdata(iindex(ipp)) >  datap  )                     &
              .OR. (      (rdata(iindex(ipp)) == datap )                      &
                    .AND. (      iindex(ipp)  >  indexp)))                CYCLE
        ENDIF
        EXIT
      ENDDO
...
#ifndef PGI_FIX_ACCROUTINE_IMPLICITNONE
IMPLICIT NONE
#endif
...
!===============================================================
! Routine (3D) optimized for vector machines:
!===============================================================

  SUBROUTINE lininterp3D_xinter1D_vec(xwerte, fwerte,   &
    ...
    REAL(KIND=wp),     INTENT(in) :: xwerte(:,:,:), fwerte(:,:,:), xinter(anzinter)
    REAL(KIND=wp),     INTENT(in), OPTIONAL :: missingval
    INTEGER, INTENT(out) :: ierr
    ...
    REAL(KIND=wp) :: xtmp, xmax(istart:iend,jstart:jend), xmin(istart:iend,jstart:jend)
    LOGICAL :: lzacc, lerr
    ...
    IF (lerr) THEN
      ierr = 2
      WRITE (*,*) &
           '%%%% lininterp3D_xinter1D: Error --- Xwerte must be monotonically increasing!'
      RETURN
    END IF
    ...
    DO n=1, anzinter
      ...
      IF (PRESENT(missingval)) THEN
        ...
        DO j=jstart,jend
          !$acc loop vector
          DO i=istart,iend
            IF (xtmp > xmax(i,j) .OR. xtmp <= xmin(i,j)) THEN
              cnt(i,j) = cnt(i,j) + 1
      ...
      ELSE
        ...
        DO j=jstart,jend
          !$acc loop vector
          DO i=istart,iend
            IF (xtmp > xmax(i,j)) THEN
              cnt(i,j) = cnt(i,j) + 1
              finter(i,j,n) = fwerte(i,j,kend)
            ELSEIF (xtmp <= xmin(i,j)) THEN
              cnt(i,j) = cnt(i,j) + 1
      ...
      DO k=kstart,kend-1
        ...
        DO j=jstart,jend
          !$acc loop vector
          DO i=istart,iend
            IF ( xtmp > xwerte(i,j,k) .AND. xtmp <= xwerte(i,j,k+1)) THEN
              cnt(i,j) = cnt(i,j) + 1
              finter(i,j,n) = fwerte(i,j,k) + (fwerte(i,j,k+1)-fwerte(i,j,k)) / &
                             (xwerte(i,j,k+1)-xwerte(i,j,k)) * (xtmp-xwerte(i,j,k))
            ENDIF
          ENDDO
        ENDDO
      ENDDO
    ...
    IF( lerr ) THEN
      ...
      WRITE (*,'(a)') '%%%% lininterp3D_xinter1D: Error --- cnt /= anzinter!'
      WRITE (*,*) '%%%% lininterp3D_xinter1D', cnt(istart:iend,jstart:jend)-anzinter
      WRITE (*,*) '%%%% lininterp3D_xinter1D', anzinter
      RETURN
    END IF
    

    
  END SUBROUTINE lininterp3D_xinter1D_vec
