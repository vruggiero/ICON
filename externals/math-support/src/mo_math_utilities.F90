...
!-----------------------------------------------------------------------
    ...
    !$ACC   PRESENT(varout) IF(acc_on)

    ! initialize c-prime and d-prime
    !$ACC PARALLEL DEFAULT(NONE) FIRSTPRIVATE(slev, elev, startidx, endidx) ASYNC(1) IF(acc_on)
...
!NEC$ outerloop_unroll(4)
    ...
    !$ACC END PARALLEL
    ! initialize varout
    !$ACC KERNELS DEFAULT(PRESENT) ASYNC(1) IF(acc_on)
