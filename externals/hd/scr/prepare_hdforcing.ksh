#!/bin/ksh
#
# prepare_hdforcing.ksh - Sub-script to run_hdmodel.ksh for preparing HD forcing data
# 
...
#         IFORCE, EXPINP, DNREMAP, GRID, YEAR, iform, DNRUN, DNDR, HDRES, FORCE_RES
...
case ${IFORCE} in
   1  )  DATA=${HDFORCING}/$EXPINP         # Input data directory 
         ...
         case $FORCE_RES in
           $HDRES ) cdo $cdocom -selvar,qs,qsb  ${DATA}/${EXPINP}_daily_${YEAR}.nc $DNRUN 
   ...
   3  )  if [ $EXPINP -le 62003 ] ; then DATA=/work/mh0231/m214046/cclm/$EXPINP ; fi
         ...
         case $EXPINP in
           ...
           6203[189] | 6204[238] ) GRIDCCLM=${HDMAIN}/grid/grid_icon_nukleus.txt ;;   # ICON-CLM Nukleus
           ...
           62033 ) GRIDCCLM=${HDMAIN}/grid/grid_cclm_cd3_ncep.txt  ;; # CoastDat3-NCEP
           6205[456] ) GRIDCCLM=${HDMAIN}/grid/grid_icon_nukleus.txt 
                   UFAK=1.666667E-4      # Despite CMOR (UFAK=0.001) erroneously in mm/(s/6)
                   cdocom=setmisstoc,0.
                   ndt_day=1  ;;   # ICON-CLM Nukleus = CCLM Nukleus
           6205[7] ) GRIDCCLM=${HDMAIN}/grid/grid_icon_nukleus.txt 
                   UFAK=0.001
                   cdocom=setmisstoc,0.
                   ndt_day=1  ;;   # ICON-CLM Nukleus = CCLM Nukleus
           62058 ) GRIDCCLM=${HDMAIN}/grid/grid_icon_nukleus.txt 
                   UFAK=1.157407E-8        # Input: mm/day --> 1/1000/86400
                   ndt_day=1  ;;   # ICON-CLM Nukleus = CCLM Nukleus
...
#        *** WRF or other RCM input
   5  )  UFAK=0.001          # Input is mm/s = kg m-2 s-1
         ndt_day=4           # Number of time steps per day
         case $EXPINP in
           61000 ) echo 'Input: WRF med grid'    ;;    # WRF med. 0.11°
           *     ) echo 'RCM (e.g. WRF) Exp. nr. not defined --> Abbruch!' ; exit ;;
         esac
         echo "Unit Factor: " $UFAK
#
#        *** Interpolation
...
##           cdo -f nc genycon,${HDMAIN}/grid/$GRID ${DATA}/WRF_Runoff_${YEAR}.nc $DNREMAP
           cdo -f nc genbil,${HDMAIN}/grid/$GRID ${DATA}/WRF_Runoff_${YEAR}.nc $DNREMAP
         fi
         cdocom="$cdocom -remap,${HDMAIN}/grid/$GRID,$DNREMAP" 

         if [ $iform -eq 0 ] ; then
           cdo -f srv $cdocom -selvar,RUNSF ${DATA}/WRF_Runoff_${YEAR}.nc $DNRUN
           cdo -f srv $cdocom -selvar,RUNSB ${DATA}/WRF_Runoff_${YEAR}.nc $DNDR 
           echo 'SRV forcing files created' 
         elif [ $iform -eq 1 ] ; then
           cdo $cdocom -chname,RUNSF,runoff -chname,RUNSB,drainage ${DATA}/WRF_Runoff_${YEAR}.nc $DNRUN 
           echo 'NetCDF forcing file created: ' $DNRUN 
         fi ;;
#
#        *** ICON forcing
   6  )  DATA=${HDFORCING}/icon/$EXPINP         # Input data directory
         UFAK=0.001         # Input is mm/s = kg m-2 s-1
         ndt_day=4          # Number of time steps per day

         if [ -e $DNREMAP ] ; then 
           echo 'Remap file exists for forcing ' $EXPINP 
         else
           cdo -f nc genycon,${HDMAIN}/grid/$GRID ${DATA}/${EXPINP}_suru_${YEAR}.nc $DNREMAP
         fi
         cdocom="$cdocom -remap,${HDMAIN}/grid/$GRID,$DNREMAP" 

         if [ $iform -eq 0 ] ; then
           echo 'SRV files are not accepted for ICON-> STOP!' ; ndt_day=-9999 ; exit
         elif [ $iform -eq 1 ] ; then
           cdo -O merge -setvar,runoff ${DATA}/${EXPINP}_suru_${YEAR}.nc \
                        -setvar,drainage ${DATA}/${EXPINP}_drain_${YEAR}.nc trun_dr.nc 
           cdo $cdocom trun_dr.nc $DNRUN
