#!/bin/ksh -l
#
# convert_hd5_discharge_to_nemo.ksh - Converts discharge on the HD grid to a selected NEMO ocean grid
# 
...
TAG_IN='hd5_1'        # Tag for discharge data
...
IBGC=2   # 1 = Discharge only, 2=1 or more tracers 3=Bias corrected discharge
#
#
# NEMO grid
case $INEMO in 
   1 ) TAG_OUT='nemo-gcoast'      # Tag for ocean model grid
       DN_HDCOUPLE="/work/gg0302/g260122/HD/input/nemo/hdcouple_${TAG_IN}_to_nemo-gcoast.nc"  ;;
   2 ) TAG_OUT='nemo-northsea'    # Tag for ocean model grid
       DN_HDCOUPLE="/work/gg0302/g260122/HD/input/nemo/hdcouple_${TAG_IN}_to_nemo-ns.nc"  ;;
   3 ) TAG_OUT='nemo-nordic'      # Tag for ocean model grid
       DN_HDCOUPLE="/work/gg0302/g260122/HD/input/nemo/hdcouple_${TAG_IN}_to_nemo-nordic.nc"  ;;
...
case $IBGC in
   ...
   * ) echo "IBGC no. $IBGC does not exist --> TERMINATE" ; exit  ;;
esac
