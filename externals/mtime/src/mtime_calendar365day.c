 ...
 * Julian Day (0,0) corresponds to Calendar-with-365-day's day 0-01-01T12:00:00.000.
 */
  ...
  if ((jd != NULL) && (d365 != NULL))
    {
      ...
      if (jday >= 0)
        {
          ...
          days = (int) (jday % NO_OF_DAYS_IN_A_YEAR_FOR_CAL_TYPE365);
      ...
      d365->time.hour = (int) jms / NO_OF_MS_IN_A_HOUR;
      d365->time.minute = ((int) jms - d365->time.hour * NO_OF_MS_IN_A_HOUR) / NO_OF_MS_IN_A_MINUTE;
      d365->time.second
          = ((int) jms - d365->time.hour * NO_OF_MS_IN_A_HOUR - d365->time.minute * NO_OF_MS_IN_A_MINUTE) / NO_OF_MS_IN_A_SECOND;
      d365->time.ms = (int) jms - d365->time.hour * NO_OF_MS_IN_A_HOUR - d365->time.minute * NO_OF_MS_IN_A_MINUTE
                      - d365->time.second * NO_OF_MS_IN_A_SECOND;
  ...
  if ((d365 != NULL) && (jd != NULL))
    {
      ...
      if (d365->date.year < 0)
        {
          jd->day = d365->date.year * NO_OF_DAYS_IN_A_YEAR_FOR_CAL_TYPE365
                    + nofDaysAfterARGMonthsInNonLeapYear[d365->date.month - 1] + d365->date.day - 1;
        }
      else
        {
          jd->day = d365->date.year * NO_OF_DAYS_IN_A_YEAR_FOR_CAL_TYPE365
